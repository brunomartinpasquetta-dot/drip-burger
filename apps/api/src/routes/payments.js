...
router.post('/webhook', async (req, res) => {
    try {
        ...
        if (patch) {
            try {
                // Si el order no tiene jornadaId pero hay jornada abierta, se la
                // asignamos en el mismo update. Sin esto el cobro queda fuera
                // del filtro del CajaCard (filter: jornadaId = X) y no aparece
                // en el resumen.
                let order = null;
                try {
